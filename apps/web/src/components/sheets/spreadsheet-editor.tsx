...
export function SpreadsheetEditor({
  ...
  const fetchRows = useCallback(
    async (page: number) => {
      setLoading(true);
      try {
        const data = await apiClient.get<RowsResponse>(
          `/api/v1/sheets/${sheetId}/rows?page=${page}&pageSize=50`
        );
        setRows(data.rows);
  ...
  const handleSubmit = async () => {
    ...
    try {
      await apiClient.post("/api/v1/change-requests", {
        sheetId,
        cells: changeList,
  ...
  const handleSaveManagers = async () => {
    ...
    try {
      ...
      await apiClient.patch(`/api/v1/sheets/${sheetId}/rows/manager`, {
        assignments,
      });
